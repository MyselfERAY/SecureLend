...
export class AdminService {
  ...
  /**
   * KYC Uyumluluk Dashboard — kimlik doğrulama durumunun platform düzeyinde özeti.
   *
   * Metrikler:
   * - Kullanıcıların NVI / KKB phone / KPS doğrulama dağılımı
   * - Mülk UAVT doğrulama dağılımı
   * - Uyumluluk riski olan kullanıcılar (herhangi bir doğrulaması eksik)
   */
  async getKycCompliance() {
    const [
      totalUsers,
      nviVerifiedUsers,
      phoneTcknVerifiedUsers,
      kpsVerifiedUsers,
      kycCompletedUsers,
      tcknEncryptedUsers,
      totalProperties,
      uavtVerifiedProperties,
      incompleteVerificationUsers,
      recentNviFailures,
    ] = await Promise.all([
      this.prisma.user.count(),
      this.prisma.user.count({ where: { nviVerified: true } }),
      this.prisma.user.count({ where: { phoneTcknVerified: true } }),
      this.prisma.user.count({ where: { kpsVerified: true } }),
      this.prisma.user.count({ where: { kycStatus: KycStatus.COMPLETED } }),
      this.prisma.user.count({ where: { tcknEncrypted: { not: null } } }),
      this.prisma.property.count(),
      this.prisma.property.count({ where: { uavtVerifiedAt: { not: null } } }),
      this.prisma.user.findMany({
        where: {
          OR: [
            { nviVerified: false },
            { phoneTcknVerified: false },
          ],
        },
        select: {
          id: true,
      ...
      this.prisma.auditLog.findMany({
        where: { action: { in: ['NVI_VERIFICATION_FAILED', 'KKB_PHONE_MISMATCH'] } },
        orderBy: { createdAt: 'desc' },
        take: 20,
      }),
    ]);
    ...
    return {
      summary: {
        totalUsers,
        nviVerified: { count: nviVerifiedUsers, pct: totalPct(nviVerifiedUsers) },
        phoneTcknVerified: { count: phoneTcknVerifiedUsers, pct: totalPct(phoneTcknVerifiedUsers) },
        kpsVerified: { count: kpsVerifiedUsers, pct: totalPct(kpsVerifiedUsers) },
        kycCompleted: { count: kycCompletedUsers, pct: totalPct(kycCompletedUsers) },
        tcknEncrypted: { count: tcknEncryptedUsers, pct: totalPct(tcknEncryptedUsers) },
      },
      properties: {
        total: totalProperties,
        uavtVerified: uavtVerifiedProperties,
        uavtVerificationRate:
          totalProperties > 0
            ? Math.round((uavtVerifiedProperties / totalProperties) * 100)
            : 0,
      },
      incompleteVerifications: incompleteVerificationUsers.map((u) => ({
        id: u.id,
        tcknMasked: u.tcknMasked,
        fullName: u.fullName,
        phone: u.phone.replace(/(\d{3})\d{3}(\d{2})(\d{2})/, '$1***$2$3'),
        nviVerified: u.nviVerified,
        phoneTcknVerified: u.phoneTcknVerified,
        kpsVerified: u.kpsVerified,
        kycStatus: u.kycStatus,
        createdAt: u.createdAt.toISOString(),
      })),
      recentFailures: recentNviFailures.map((a) => ({
        action: a.action,
        tcknMasked: a.tcknMasked,
