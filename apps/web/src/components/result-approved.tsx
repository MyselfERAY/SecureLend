      ...
      <div className="w-full max-w-xl animate-fade-up rounded-3xl border border-slate-200 bg-white/95 p-8 shadow-card sm:p-10">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-4 font-display text-3xl font-extrabold tracking-tight text-slate-900">Başvuru sonucu olumlu</h1>
            <p className="mt-2 text-sm text-slate-600">TCKN: {application.maskedTckn}</p>
          </div>
          <div className="inline-flex h-12 w-12 animate-scale-in items-center justify-center rounded-full bg-emerald-100 text-emerald-700">
...
function DataCard({ label, value, tone = 'default' }: { label: string; value: string; tone?: 'default' | 'success' }) {
  return (
    <div className={`rounded-xl border p-4 ${tone === 'success' ? 'border-emerald-200 bg-emerald-50/70' : 'border-slate-200 bg-slate-50'}`}>
      ...
      <div className="mt-2 font-display text-2xl font-extrabold tracking-tight text-slate-900">{value}</div>
    </div>
  );
}
