...
export default function AdminArticlesPage() {
  ...
  const fetchDrafts = async () => {
    ...
    setLoading(true);
    const res = await api<Article[]>('/api/v1/articles/admin/drafts', { token: tokens.accessToken });
    if (res.status === 'success' && res.data) setDrafts(res.data);
    setLoading(false);
  };

  useEffect(() => { fetchDrafts(); }, [tokens?.accessToken]);
  ...
  const handleDelete = async (id: string) => {
    if (!tokens?.accessToken || !confirm('Bu taslağı silmek istediğinize emin misiniz?')) return;
    setActionLoading(id);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900">Makale Taslakları</h1>
          <p className="mt-1 text-sm text-slate-500">
            Marketing Agent'ın ürettiği taslakları inceleyin ve yayınlayın.
          </p>
        </div>
        {drafts.length > 0 && (
          ...
          <div className="rounded-2xl border border-slate-200 bg-white">
            {!preview ? (
              <div className="flex h-full min-h-[300px] items-center justify-center text-sm text-slate-400">
                Önizlemek için sol taraftan bir taslak seçin.
              </div>
            ) : (
              <div className="p-6 space-y-4">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-semibold text-amber-700">Taslak</span>
